...
class DatabaseManager:
    ...
    def _initialize_db(self):
        ...
        CREATE TABLE IF NOT EXISTS conversations (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            user_id INTEGER NOT NULL,
            agent_type TEXT NOT NULL, -- 小账/明查/省省
...
class FinanceTools:
    ...
    def track_goal_progress(self, goal_name: str = None):
        ...
        """
        # 由于数据库中没有目标表，这里返回模拟数据
        # 在实际应用中，应该从数据库的目标表中获取数据

        current_savings = 0  # 这里应该从数据库查询实际储蓄
        total_goals = [
            {
                "goal_name": "应急基金",
                "target_amount": 10000,
...
class FinanceAgent(ChatAgent):
    def __init__(self, system_msg, model, tools, db_manager, user_id, agent_type):
        ...
        self.db_manager = db_manager
        self.user_id = user_id
        self.agent_type = agent_type  # 区分不同Agent类型：小账/明查/省省
    ...
    def _process_tool_results(self, response):
        ...
        try:
            ...
            # 简单处理工具结果，实际应用中可以根据不同工具和结果类型进行更复杂的处理
            result_str = ""
            ...
            original_content = response.msgs[0].content if hasattr(response, 'msgs') and response.msgs else (response.content if hasattr(response, 'content') else '无回复内容')
...
def create_recorder_agent(model, db_manager, user_id):
    ...
    system_msg = BaseMessage(
        ...
        content="""
# 角色
你名为“小账”，是用户专属且极为专业的记账官😎，始终坚守精准、高效的理念，迅速且准确地处理用户的收支记录信息。
...
def create_analyzer_agent(model, db_manager, user_id):
    ...
    system_msg = BaseMessage(
        ...
        role_type=RoleType.ASSISTANT,
        content="""
你叫“明查”，如同一位超厉害的财务侦探🕵️‍，是用户专属的财务洞察官...
        """,
        meta_dict={}
    )
...
def create_saver_agent(model, db_manager, user_id):
    ...
    system_msg = BaseMessage(
        ...
        content="""
# 角色
你叫"省省"，是一位活力满满、超爱分享省钱秘籍的省钱小能手🧑‍✈️...
        """,
        meta_dict={}
    )
...
def create_planner_agent(model, db_manager, user_id):
    ...
    system_msg = BaseMessage(
        ...
        content="""
# 角色
你叫"远谋"，是一位经验丰富的财务规划专家🏆，擅长帮助用户设定合理的财务目标并制定可行的实现计划。
...
5. 根据需要协调其他Agent提供支持（如请求省省提供针对性省钱建议）
...
class FinanceInterface:
    ...
    def _handle_recorder_chat(self, message, chat_history):
        ...
        try:
            ...
            # 生成回复
            user_msg = BaseMessage.make_user_message(role_name="用户", content=message, meta_dict={})
